...
Official @EliteMusicApiBot runs with Master Unlimited access.
Cloned Bots run on Free Quota Tier (50 songs/day, 1 concurrent VC).
"""
import hashlib
import secrets
...
async def get_security_context(
    ...
    # 1. Master / Official Bot Key -> 100% UNLIMITED
    ...
    # 2. Dynamic Clone Key (clone_<bot_token_id>) -> Free Tier Quota (50 tracks/day, 1 VC)
    if x_api_key.startswith("clone_"):
        return SecurityContext(
            api_key=x_api_key,
            key_hash=hash_api_key(x_api_key),
            tier="tier_free",
